...
public class JSR385UserType implements CompositeUserType {
    ...
    public Object getPropertyValue(Object component, int property) {
        if (component instanceof Quantity<?> piece) {
    ...
    public void setPropertyValue(Object component, int property, Object value) {
        if (value instanceof Quantity<?> piece) {
    ...
    public Object nullSafeGet(ResultSet rs, String[] names, SharedSessionContractImplementor session, Object owner) throws SQLException {
        ...
        if (WMSUnits.exists(unit, unitTypeClass)) {
            return WMSUnits.getQuantity(rs.getDouble(names[1]), unit);
        }
    ...
    public void nullSafeSet(PreparedStatement st, Object value, int index, SharedSessionContractImplementor session) throws SQLException {
        if (value == null) {
            ...
            if (value instanceof Quantity qty) {
                if (UnitDimension.NONE.equals(qty.getUnit().getDimension())) {
                    st.setString(index, "PC@" + Each.class.getCanonicalName());
                    String amount = WMSUnits.getQuantity(qty.getValue().doubleValue(), "PC").toString();
                    st.setString(index + 1, amount);
                    if (LOGGER.isTraceEnabled()) {
                        LOGGER.trace("Binding [PC@{}] to parameter [{}]", Each.class.getCanonicalName(), index);
